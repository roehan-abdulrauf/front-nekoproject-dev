...
const HomeGroups = ({ navigation }) => {
  ...
  const [activeScreen, setActiveScreen] = useState('homeGroups');
  ...
  const handleUserPress = (chat) => {
    navigation.navigate('ChatRoomScreen', { chatRoomId: chat._id, chatRoomName: chat.chatRoomName, initiatorPseudo: chat.initiatorPseudo, });
  };
  ...
  return (
    <MenuProvider style={styles.menuProvider}>
      ...
      <View style={styles.chatRoomHome}>
        <Text style={styles.screenName}>Espaces</Text>
      </View>
      ...
      <View style={styles.buttonContainerFlottant}>
        <TouchableOpacity style={styles.addButton} onPress={() => navigation.navigate('CreateGroupMemberScreen')}>
          <Icon name="group-add" size={20} />
          <Text style={{ fontSize: 12, paddingLeft: 4, }}>Nouvel espace</Text>
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  body: {
    flexGrow: 1,
    // paddingTop: 10,
    paddingBottom: 80, // Hauteur maximale de la partie corps
    minHeight: 600,
  },
  headerTitle: {
    color: '#FFFFFF',
